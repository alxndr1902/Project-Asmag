...
public class LoanServiceImpl extends BaseService implements LoanService {
    ...
    private final LoanDetailDao loanDetailDao;
    private final AssetDao assetDao;
    private final LocationDao locationDao;
    private final EmployeeDao employeeDao;

    public LoanServiceImpl(LoanDao loanDao, LoanDetailDao loanDetailDao, AssetDao assetDao, LocationDao locationDao, EmployeeDao employeeDao) {
    ...
    public UpdateResponseDTO returnAsset(String id, List<String> loanDetailIdList) {
        ...
        for (String loanDetailId : loanDetailIdList) {
            LoanDetail loanDetail = loanDetailDao.findById(UUID.fromString(loanDetailId)).orElseThrow(
    ...
    private void setTarget(CreateLoanRequestDTO request, Loan loan) {
        if (request.getAssetTargetId() == null && request.getLocationTargetId() == null) {
            Employee employee = employeeDao.findById(UUID.fromString(request.getAssetTargetId())).orElseThrow(
        ...
        if (request.getAssetTargetId() == null && request.getEmployeeTargetId() == null) {
            Location location = locationDao.findById(UUID.fromString(request.getLocationTargetId())).orElseThrow(
        ...
        if (request.getEmployeeTargetId() == null && request.getLocationTargetId() == null) {
            Asset asset = assetDao.findById(UUID.fromString(request.getAssetTargetId())).orElseThrow(
